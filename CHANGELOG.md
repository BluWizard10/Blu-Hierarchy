...
- **Added Rounded Rectangle renderer for Tags and Layers**
  ...
  - Text is automatically contrasted based on the color used.
- Removed Unity Layer Icons, changed to graphical Text display.
  - *I've decided that Icons cannot be recognized well without a Tooltip. While I would add one, I think it's better if the text shows instead, as stated above. So, this update introduces that change.*

# [1.11.0]
- **Added Relationship Lines.**
  - *This was highly requested! After so much trial and error, I have finally designed a solution that looks clean and functions reliably. Enjoy!*
  - This feature draws a tiny line of 1 pixel wide that shows the relationship between parents and children objects.
  - Enabled by default. Can be toggled off in Settings via `Tools -> BluWizard LABS -> Enhanced Hierarchy Settings -> Show Relationship Lines`.
